...
class RegionProposalNetwork(nn.Module):
    ...
    def generate_anchors(self, image, feature_map):        
        ...
        ws =(w[:, None] * scales[None, :]).view(-1) # get width of unit area anchors in all different scales
        hs =(h[:, None] * scales[None, :]).view(-1) # get height of unit area anchors in all different scales
        
        # give the width and height of all 9 anchor boxes 
        # All those boxes are centered at (0,0) 
        # eg: those base_anchors will be shape 9x4 
        base_anchors = torch.stack([-ws, -hs, ws, hs], dim=1)/2 
        base_anchors =base_anchors.round()
        
        # Get the shift in x axis
